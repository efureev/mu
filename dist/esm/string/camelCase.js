...
const camelCase = (input, options) => {
  ...
  let str = typeof input === 'string' ? input.trim() : input.map(x => x.trim()).filter(x => x.length).join('-');
  ...
  str = str.replace(/^[ ._\-]+/, '').toLowerCase().replace(/[ ._\-]+(\w|$)/g, (_, p1) => p1.toUpperCase()).replace(/\d+(\w|$)/g, m => m.toUpperCase());
