...
describe('remove by string key', () => {
  ...
  it('basic', () => {
    ...
    }, 'items.items.k')).toEqual({label: 'Root', items: {k: 1, v: 2, items: {v: 4}}})
...
describe('remove by array keys', () => {
  ...
  it('array', () => {
    ...
    expect(remove({label: 'Root', items: {k: 1, v: 2}}, ['items.k', 'items.v'])).toEqual({label: 'Root', items: {}})

    expect(remove({label: 'Root', items: {k: 1, v: 2, items: {k: 3, v: 4}}},
      ['items.items.k', 'items.k']))
      .toEqual({label: 'Root', items: {v: 2, items: {v: 4}}})

    expect(remove({label: 'Root', items: {k: 1, v: 2, items: [{k: 1, v: 2}, {k: 3, v: 4}]}}, ['items.items.0.k']))
      .toEqual({label: 'Root', items: {k: 1, v: 2, items: [{v: 2}, {k: 3, v: 4}]}})

    expect(remove({label: 'Root', items: {k: 1, v: 2, items: [{k: 1, v: 2}, {k: 3, v: 4}]}}, ['items.items.0']))
      .toEqual({label: 'Root', items: {k: 1, v: 2, items: [{k: 3, v: 4}]}})

    expect(remove({
      label: 'Root',
      items: {k: 1, v: 2, items: [{k: 1, v: 2}, {k: 3, v: 4}]},
